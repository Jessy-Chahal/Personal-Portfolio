...
const TEXT_ZYXYFY = 'ZYXIFY'
const TEXT_JESSY = 'Jessy'
const CHARS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*'

export default function Logo() {
  const [displayText, setDisplayText] = useState(TEXT_ZYXYFY)
  const [currentText, setCurrentText] = useState(TEXT_ZYXYFY) // Track which text is currently displayed
  const shuffleIntervalRef = useRef<NodeJS.Timeout | null>(null)
  const completeIntervalRef = useRef<NodeJS.Timeout | null>(null)
  const iterationRef = useRef(0)
  const isHoveringRef = useRef(false)
  const targetTextRef = useRef<string>(TEXT_ZYXYFY) // Track the target we're animating to
  ...
  const handleMouseEnter = () => {
    ...
    const targetText = currentText === TEXT_ZYXYFY ? TEXT_JESSY : TEXT_ZYXYFY
    ...
    shuffleIntervalRef.current = setInterval(() => {
      ...
      if (targetRevealed >= targetText.length) {
        ...
        setDisplayText(targetText)
        setCurrentText(targetText) // Update current text
        targetTextRef.current = targetText
      }
    }, 50)
  }
